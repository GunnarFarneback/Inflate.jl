...
const fixed_literal_or_length_table = Vector{UInt16}[
    [], [], [], [], [], [],
    [0x100:0x117;], # 7-bit
    [0x000:0x08f; 0x118:0x11f], # 8-bit
    [0x090:0x0ff;]] # 9-bit

const fixed_distance_table = Vector{UInt16}[[], [], [], [], [0x000:0x01f;]]
...
mutable struct InflateData <: AbstractInflateData
    ...
    bytepos::Int
    bitpos::Int
    literal_or_length_code::Vector{Vector{UInt16}}
    distance_code::Vector{Vector{UInt16}}
...
"""
    get_value_from_code(data::AbstractInflateData, code) -> UInt16
"""
function get_value_from_code(data::AbstractInflateData,
                             code::Vector{Vector{UInt16}})
    v = 0
    for c in code
        v = (v << 0x1) | getbit(data)
        if v < length(c)
...
"""
    get_literal_or_length(data::AbstractInflateData) -> UInt16
"""
function get_literal_or_length(data::AbstractInflateData)
    return get_value_from_code(data, data.literal_or_length_code)
end

const base_length = Tuple(UInt8[11:2:17; 19:4:31; 35:8:59; 67:16:115; 131:32:227])
...
function getlength(data::AbstractInflateData, v::Int)
    if v <= 264
        return v - 254
    elseif v <= 284
        extra_length_bits = (v - 264 + 3) >> 0x2
        return base_length[v - 264] + getbitsint(data, extra_length_bits)
    else
        return 258
    end
end
...
function getdist(data::AbstractInflateData)
    b = Int(get_value_from_code(data, data.distance_code))
    if b <= 3
        return b + 1
    else
        extra_bits = (b - 2) >> 0x1
        return 1 + ((2 + b & 0x1) << extra_bits) + getbitsint(data, extra_bits)
    end
end

"""
    transform_code_lengths_to_code(code_lengths::Vector{Int}) -> Vector{Vector{UInt16}}
"""
...
function read_code_tables(data::AbstractInflateData)
    ...
    for i = 1:hclen
        code_length_code_lengths[1 + order[i]] = getbits(data, 3) % Int
    end
...
"""
    update_adler(adler::UInt32, x::UInt8) -> UInt32
"""
function update_adler(adler::UInt32, x::UInt8)
    s2, s1 = adler >> 0x10, adler & 0xffff
    s1 += x
    if s1 >= UInt32(65521)
        s1 -= UInt32(65521)
    end
...
const crc_table = [make_crc_table(n) for n = 0x00:0xff]
...
function get_input_byte(data::StreamingInflateData)
    if data.input_buffer_pos > length(data.input_buffer)
        data.input_buffer = read(data.stream, buffer_size)
